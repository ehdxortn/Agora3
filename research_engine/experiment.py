...
class BTCDataLoader:
 ...
 async def load(self):
  ...
  if not candles: raise RuntimeError("candles_4h contains no BTCUSDT rows")
  ...
  if futures:
   ...
   for c in [x for x in f.columns if x not in {"symbol","bucket_open","archive_date","source_sha256","source_url","collected_at"}]: f[c]=pd.to_numeric(f[c],errors="coerce")
   f=f.sort_values("bucket_open").drop_duplicates("bucket_open",keep="last"); df=df.merge(f.drop(columns=["symbol"],errors="ignore"),left_on="open_time",right_on="bucket_open",how="left")
  if funding:
   f=pd.DataFrame(funding); f["funding_time"]=pd.to_datetime(f["funding_time"],utc=True); f["funding_rate"]=pd.to_numeric(f["funding_rate"],errors="coerce"); f=f.sort_values("funding_time").drop_duplicates("funding_time",keep="last")
   df=pd.merge_asof(df.sort_values("decision_time"),f[["funding_time","funding_rate"]],left_on="decision_time",right_on="funding_time",direction="backward",allow_exact_matches=True)
  if onchain:
   o=pd.DataFrame(onchain); o["day"]=pd.to_datetime(o["day"],utc=True)
   for c in o.columns:
    if c not in {"day","source","source_version","source_query_sha256","collected_at","created_at","is_backfill"}: o[c]=pd.to_numeric(o[c],errors="coerce")
   o["available_at"]=o["day"]+pd.Timedelta(days=1); keep=[c for c in o.columns if c not in {"day","source","source_version","source_query_sha256","collected_at","created_at","is_backfill"}]+["available_at"]
...
class ExperimentRunner:
 ...
 def run(self,spec,stability=True):
  sh=hashlib.sha256(spec.model_dump_json().encode()).hexdigest(); eid=f"EXP-{uuid4().hex[:12]}"; df=self.feature_engine.apply(self.raw,spec.features); names=[f.name for f in spec.features]; u=df.dropna(subset=names+["open","high","low","close"]).reset_index(drop=True)
  ...
  th=self._resolve_thresholds(u.iloc[slices[0][0]:slices[0][1]],spec.conditions); sig=self._build_signal(u,spec.conditions,th); allr=self._simulate(u,sig,spec); rs=[self._returns_for_range(allr,*x) for x in slices]
  flags=[]
  if len(rs[2])<30: flags.append("LOW_OOS_SAMPLE")
  if rs[0] and rs[2] and np.mean(rs[0])>0>=np.mean(rs[2]): flags.append("OOS_SIGN_FLIP")
  if rs[1] and rs[2] and np.mean(rs[1])*np.mean(rs[2])<0: flags.append("VALIDATION_TEST_INSTABILITY")
  r=ExperimentResult(experiment_id=eid,spec_hash=sh,train=self._metrics(rs[0],slices[0][1]-slices[0][0]),validation=self._metrics(rs[1],slices[1][1]-slices[1][0]),test=self._metrics(rs[2],slices[2][1]-slices[2][0]),methodological_flags=flags)
  if stability:r.parameter_stability=self._stability_sweep(spec)
  r.passed_minimum_gate=self._minimum_gate(r); return r
 ...
 def _simulate(self,df,signal,spec):
  ...
  while i<len(df)-2:
   ...
   ei=i+1; entry=float(df.iloc[ei].open); mx=min(len(df)-1,ei+spec.exit.horizon_bars-1); gross=None; xi=mx
   if spec.exit.mode=="stop_target":
    ...
    for j in range(ei,mx+1):
     ...
     stop=(l<=entry*(1-sl)) if spec.side=="long" else (h>=entry*(1+sl)); target=(h>=entry*(1+tp)) if spec.side=="long" else (l<=entry*(1-tp))
 ...
 def _stability_sweep(self,spec):
  vs=[]
  for d in (-.05,.05):
   ...
   if changed:
    try:
     r=self.run(c,False); vs.append({"delta":d,"test_trades":r.test.trades,"test_mean_net_return":r.test.mean_net_return,"test_profit_factor":r.test.profit_factor})
    except Exception as e:vs.append({"delta":d,"error":str(e)})
  pos=[v for v in vs if (v.get("test_mean_net_return") or 0)>0]; return {"variants":vs,"positive_neighbor_fraction":len(pos)/len(vs) if vs else None}
 @staticmethod
 def _minimum_gate(r):
  t=r.test; st=r.parameter_stability.get("positive_neighbor_fraction")
  return bool(t.trades>=80 and (t.mean_net_return or 0)>0 and (t.profit_factor or 0)>1.10 and (t.max_drawdown is not None and t.max_drawdown>=-.25) and (st is None or st>=.5))
