...
def test_holdout_is_sealed_by_default():
 r=ExperimentRunner(data()).run(spec(),stability=False);assert r.holdout_revealed is False;assert r.gate_basis=='validation';assert r.test is None;assert 'SEALED_HOLDOUT_NOT_EVALUATED' in r.methodological_flags
def test_holdout_can_be_revealed_explicitly():
 a=ExperimentRunner(data());r=a.run(spec(),stability=False);x=a.run(spec(),stability=False,reveal_holdout=True,experiment_id=r.experiment_id);assert x.holdout_revealed is True;assert x.gate_basis=='test';assert x.experiment_id==r.experiment_id;assert x.test is not None
