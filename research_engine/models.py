...
class ExperimentSpec(BaseModel):
 hypothesis:str=Field(min_length=20,max_length=2000);mechanism:str=Field(min_length=20,max_length=4000);side:Literal["long","short"];features:list[FeatureSpec]=Field(min_length=1,max_length=12);conditions:list[ConditionSpec]=Field(min_length=1,max_length=12);exit:ExitSpec=Field(default_factory=ExitSpec);train_fraction:float=Field(default=.60,ge=.4,le=.8);validation_fraction:float=Field(default=.20,ge=.1,le=.3);embargo_bars:int=Field(default=6,ge=0,le=84);fee_bps:float=Field(default=8,ge=0,le=100);slippage_bps:float=Field(default=2,ge=0,le=100);source_research_ids:list[str]=Field(default_factory=list,max_length=30);novelty_note:str=""
 @model_validator(mode="after")
 def valid(self):
  if self.train_fraction+self.validation_fraction>=.95:raise ValueError("Need at least 5% sealed holdout data")
...
class StartRunRequest(BaseModel):mission:str="Discover repeatable, causal, economically exploitable Bitcoin trading edges that remain positive after realistic fees and slippage and survive chronological out-of-sample validation.";budget_usd:float|None=Field(default=None,gt=0,le=1000);notes:str=""
