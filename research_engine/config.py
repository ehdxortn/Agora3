...
class Settings:
 ...
 daily_budget_usd:float=_float("DAILY_BUDGET_USD",15.0);run_budget_usd:float=_float("RUN_BUDGET_USD",10.0);max_cycles_per_job:int=_int("MAX_CYCLES_PER_JOB",100);literature_min_sources:int=_int("LITERATURE_MIN_SOURCES",120);max_holdout_evaluations:int=_int("MAX_HOLDOUT_EVALUATIONS",12);max_web_search_calls:int=_int("MAX_WEB_SEARCH_CALLS",8);min_experiments_before_global_stop:int=_int("MIN_EXPERIMENTS_BEFORE_GLOBAL_STOP",12)
 literature_retrieval_limit:int=_int("LITERATURE_RETRIEVAL_LIMIT",18);failure_retrieval_limit:int=_int("FAILURE_RETRIEVAL_LIMIT",16);topic_reuse_min_sources:int=_int("TOPIC_REUSE_MIN_SOURCES",6);literature_map_refresh_delta:int=_int("LITERATURE_MAP_REFRESH_DELTA",20)
 round_trip_fee_bps:float=_float("ROUND_TRIP_FEE_BPS",8.0);slippage_bps:float=_float("SLIPPAGE_BPS",2.0);min_oos_trades:int=_int("MIN_OOS_TRADES",80);rare_event_min_validation_trades:int=_int("RARE_EVENT_MIN_VALIDATION_TRADES",20);walk_forward_folds:int=_int("WALK_FORWARD_FOLDS",3);min_positive_wf_fraction:float=_float("MIN_POSITIVE_WF_FRACTION",0.67);min_event_clusters:int=_int("MIN_EVENT_CLUSTERS",12);cost_stress_bps:float=_float("COST_STRESS_BPS",20.0);max_drawdown_gate:float=_float("MAX_DRAWDOWN_GATE",0.25);onchain_lag_days:int=_int("ONCHAIN_LAG_DAYS",2)
